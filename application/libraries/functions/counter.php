...
class Counter_Functions {
    ...
    public function totalHits() {
        $lf_name = "hits.txt";
        $monthly = 0;  // Save new log file each month 0:No 1:Yes
        $monthly_path = "oldfiles"; // Path to store old files: oldfiles/6-12.txt
        $type = 2; // Count unique hits or all hits: 0 = All hits 1 = Unique hits 2 = Both
        $beforeAllText = "Hits: ";
        $beforeUniqueText = " ";
        $display = 1; // Display hits on this page:
        $separator = " from ";

        $log_file = dirname(__FILE__) . '/' . $lf_name;

//Check for "?display=true" in URL.
        if (isset($_GET['display']) == "true") {
            //Show include() info.
